...
namespace LeetCodeNote.Array
{
    ...
    public class Solution_15
    {
        ...
        // (2025/2/7) method My 0-我的题解0
        // 先排序，因为a+b+c=0，所以双指针打头来计算a+b，c用二分缩小查找范围
        // 失败！超出时限
        public IList<IList<int>> ThreeSum_My_0(int[] nums)
        {
            ...
            for (int i = 0; i < nums.Length; i++)
            {
                for (int j = i + 1; j < nums.Length; j++)
                {
                    ...
                    while (start <= end)
                    {
                        ...
                        {
                            List<int> list = new List<int> { nums[i], nums[j], t };
                            if (CanAdd(res, list))
                            {
                                res.Add(list);
                            }
        ...
        private bool CanAdd(IList<IList<int>> res, List<int> checklist)
        {
            foreach (IList<int> list in res)
            {
                if (list.SequenceEqual(checklist))
                {
                    return false;
                }
            }
        ...
        // (2025/2/7) method My 1-我的题解1
        // 看了官方题解后，在“我的题解0”基础上的优化：优化了本层循环可能出现重复元素的情况，这样后面就不用再额外判断是否可加进列表。
        // 二分比官方题解快9倍！“我的题解1”用了199ms，官方题解用了1811ms。
        // 时间复杂度：O(N*N*logN)，空间复杂度：O(NlogN)
        public IList<IList<int>> ThreeSum_My_1(int[] nums)
        {
            IList<IList<int>> res = new List<IList<int>>();
            // 排序，方便后面计算
            System.Array.Sort(nums);
        ...
        // (2025/2/7) method My 2-我的题解2
        // 在看了网友题解（https://leetcode.cn/problems/3sum/solutions/1968332/shuang-zhi-zhen-xiang-bu-ming-bai-yi-ge-pno55/）（如下“method Other 0”）后，在“我的题解1”再加上两点优化
        // 用了172ms
        public IList<IList<int>> ThreeSum_My_2(int[] nums)
        {
            IList<IList<int>> res = new List<IList<int>>();
            // 排序，方便后面计算
            System.Array.Sort(nums);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    // 优化点1：因为nums已经排序，如果连最小的前三项和都比0大，那这个数组就不用再计算了，毕竟后面的值肯定会越来越大
                    break;
                }
                if (nums[i] + nums[n - 1] + nums[n - 2] < 0)
                {
                    // 优化点2：因为nums已经排序，如果连本层最小项与最大两项和比0小，那本层最小项也太小，直接前进
                    continue;
                }
                if (i > 0 && nums[i] == nums[i - 1])
                {
        ...
        // (2025/2/7) method Other 0-排名第一的网友题解！
        // 用了34ms！
        public IList<IList<int>> ThreeSum_Other_0(int[] nums)
        {
            ...
            for (int i = 0; i < n - 2; i++)
            {
                ...
                while (left < right)
                {
                    int n2 = nums[left];
                    int n3 = nums[right];
                    int sum = n1 + n2 + n3; // n1固定不动，调整n2和n3的值
                    if (sum > 0)
                    {
                        right--;
                    }
